...
function rankForInjection(item, profile) {
  ...
  const abstract = (typeof (item.abstract || item.overview || "") === "string" ? (item.abstract || item.overview || "") : String(item.abstract || item.overview || "")).trim();
...
function dedupeByAbstract(items) {
  ...
  return items.filter(item => {
    const key = (typeof (item.abstract || item.overview || "") === "string" ? (item.abstract || item.overview || "") : String(item.abstract || item.overview || "")).trim().toLowerCase() || item.uri;
...
function postProcess(items, limit, threshold) {
  ...
  for (const item of sorted) {
    ...
    const abs = (typeof (item.abstract || item.overview || "") === "string" ? (item.abstract || item.overview || "") : String(item.abstract || item.overview || "")).trim().toLowerCase();
